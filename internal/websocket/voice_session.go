...
// Snapshot returns a deep copy of the voice session state, safe to read
// without holding the session lock. Returns a pointer to a freshly
// constructed VoiceSession so no mutex is ever copied by value (go vet
// copylocks).
